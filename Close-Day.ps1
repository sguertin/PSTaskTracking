function Close-Day {
    ...
    if ([string]::IsNullOrEmpty($mdToPdfCmd)) {
        Write-PSHost "COPY $reportFileName ====> $outputDirectory" -Command $MyInvocation.MyCommand;
        ...
        Write-PSHost "> $outputCommand" -Command $MyInvocation.MyCommand;
        Invoke-Expression -Command $outputCommand | Out-Null;
        Write-PSHost "$outputFileName written to '$outputDirectory'" -Command $MyInvocation.MyCommand;
    }
}
